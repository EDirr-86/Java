...
public class Fventa extends javax.swing.JDialog {
int aux; String arreglo[];
  Calendar cal=Calendar.getInstance(); 
    ...
    public void validarvaciosProd(Object[] datos) {
     ...
     public void validarvacios(Object[] datos) {
        for (int i = 0; i <= datos.length - 1; i++){
            if (datos[i].toString().isEmpty()){
                if(jCbxIva.getSelectedIndex()==0&&jCbxClie.getSelectedIndex()==0){;aux=1;}}}}
  ...
  public void habilitarB()
   ...
   this.jCbxClie.setSelectedIndex(0);this.jCbxClie.setEnabled(false);this.jBtnConfirm.setVisible(true);this.jBtnGrabar.setEnabled(false);
  ...
  public void limpiar()
   ...
   this.jCbxClie.setSelectedIndex(0);this.jCbxClie.setEnabled(true);this.jBtnConfirm.setVisible(false);this.jBtnGrabar.setEnabled(true);
   ...
   tabla.repaint();tabla.revalidate(); txtfec.setText(aaaa+"-"+mm+"-"+dd);
    ...
    public Fventa(javax.swing.JDialog parent, boolean modal) {
        super(parent, modal);
        initComponents();
        ...
        etidclie.setVisible(false); etidfvent.setVisible(false);
        
        try 
         {
         conexion miconexion = new conexion();
         String consprov = "select nombre_cl from cliente where estado_cl= 'activo' order by nombre_cl";
         ResultSet consulta =miconexion.consulta(consprov);
         int i=0;
         do{consulta.next();
         this.jCbxClie.addItem(consulta.getString(1));
    ...
    private void initComponents() {
        ...
        etidclie = new javax.swing.JLabel();
        etidfvent = new javax.swing.JLabel();
        ...
        jPanel1.setBorder(javax.swing.BorderFactory.createTitledBorder(null, "FACTURA DE VENTA: ", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Tahoma", 1, 12))); // NOI18N
        jPanel1.setForeground(new java.awt.Color(255, 255, 255));

        etid.setText("Codigo:");

        etfec.setText("Fecha de la Venta:");
        ...
        etidclie.setText("etidiprov");

        etidfvent.setText("etidocomp");
            ...
            .addGroup(jPanel1Layout.createSequentialGroup()
                ...
                .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addGroup(jPanel1Layout.createSequentialGroup()
                        ...
                        .addGap(18, 18, 18)
                        .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            .addComponent(etidfvent)
                            .addGroup(jPanel1Layout.createSequentialGroup()
                                .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING, false)
                                    .addComponent(txtid_buscar, javax.swing.GroupLayout.DEFAULT_SIZE, 144, Short.MAX_VALUE)
                                    .addComponent(txtfec)
                                    .addComponent(jCbxClie, 0, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE))
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                                .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                                    .addComponent(etidclie)
 ...
 try{   conexion miconexion = new conexion();
        ...
        String ID = "SELECT MAX( (idventa) +1 )FROM ventas";
        ResultSet consulta =miconexion.consulta(ID);
        consulta.next();
        if (consulta.getString(1)==null){etidfvent.setText(""+1);}
        else
        etidfvent.setText(""+consulta.getString(1));consulta.close();
 }
 catch (Exception e)
        {
       JOptionPane.showMessageDialog(null, e.getMessage());
 ...
 try{
     conexion miconexion = new conexion();   
     String inscomp = "insert into ventas values (?,?,?)";
     ...
     validarvacios(datos);
        if(aux!=1){
            miconexion.psPrepararSentencias.setString(1, etidfvent.getText());
            miconexion.psPrepararSentencias.setString(2,etidclie.getText());
            miconexion.psPrepararSentencias.setString(3, txtfec.getText());
            miconexion.psPrepararSentencias.executeUpdate();    
            miconexion.miconexion.close();
            JOptionPane.showMessageDialog(null, "Venta cargada");
                }
 ...
 try{
        conexion miconexion = new conexion ();
        String insertt = "insert into venta_detalle values (?,?,?,?)";
...
try{
    conexion miconexion = new conexion();
    String stock="select cantidad_pr from producto where idprod = (select idprod from producto where detalle_pr ='"+jCbxProd.getSelectedItem()+"')";
    ResultSet consulta = miconexion.consulta(stock);
    consulta.next();
    int asd = Integer.parseInt(txtcan.getText());
    
    if(consulta.getInt(1)>=asd){
        if (aux != 1) {
            ...
            datos[3] =  String.valueOf(Double.parseDouble(txtcan.getText())* Double.parseDouble(txtprec.getText())) ;
                try{
                    ...
                    ResultSet consulta2 = miconexion.consulta(cons);
                        if(consulta2.last()){datos[4]=consulta2.getString(1).toString();}
        ...
        else{JOptionPane.showMessageDialog(null, "Se encuentran campos vacios");aux=0;}}
    else{JOptionPane.showMessageDialog(null, "No se puede realizar la venta del prododucto, solo puede vender "+consulta.getString(1));}}
catch(Exception e){JOptionPane.showMessageDialog(null, e.getMessage());}
...
try{
conexion miconexion = new conexion ();
String cons = "select idcliente,fecfac_ve from ventas where idventa = " + txtid_buscar.getText();
...
cons = "select nombre_cl from cliente where idcliente = " + aux;
                    consulta =miconexion.consulta(cons);
                    if(consulta.last()){String temp=consulta.getString(1);
                    int pos=metodo.posicionar(arreglo, temp);
                    this.jCbxClie.setSelectedIndex(pos);
                    }
                    

cons = "select * from venta_detalle where idventa = " + txtid_buscar.getText();
                    consulta =miconexion.consulta(cons);
                    String datos[]=new String [5];
                    double total=0;
                    do 
        ...
        else{JOptionPane.showMessageDialog(null,"No se ha encontrado ninguna factura de venta!");} 
       }
catch (Exception e)
{
JOptionPane.showMessageDialog(null, e.getMessage());
...
if(this.jCbxClie.getSelectedIndex()>0){
        try{ conexion miconexion = new conexion();
            String cons = "select direccion_cl,telefono_cl,cuit_cl,idiva,idcliente from cliente where nombre_cl='"+jCbxClie.getSelectedItem()+"'";
            ResultSet consulta =miconexion.consulta(cons);
            consulta.next();
            ...
            this.etidclie.setText(""+consulta.getString(5));
}
            catch (Exception e)
        {JOptionPane.showMessageDialog(null, e.getMessage());}        
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            ...
            public void run() {
                Fventa dialog = new Fventa(new javax.swing.JDialog(), true);
    ...
    private javax.swing.JLabel etidclie;
    private javax.swing.JLabel etidfvent;
